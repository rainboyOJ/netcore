...
namespace netcore {
    ...
    public:
        ...
        /**
         * @brief 根据request 判断是否是 需要升级成 websocket 升级
         */
        ...
        */
        ...
        int parse_header(const char* buf, size_t size) {
            ...
            left_header_len_ = 0;
            int header_len_ = -1; //没有得到 确定的长度
            if (length_field <= 125) {
                payload_length_ = length_field;
                header_len_     = SHORT_HEADER;
            }
            else if (length_field == 126)  //msglen is 16bit!
            {
                payload_length_ = ntohs(*(uint16_t *)&inp[2]);// (inp[2] << 8) + inp[3];
                pos += 2;
                left_header_len_ = size >=MEDIUM_HEADER ? 0 : MEDIUM_HEADER - size;
                header_len_      = MEDIUM_HEADER;
            }
            else if (length_field == 127)  //msglen is 64bit!
            {
                payload_length_ = (size_t)be64toh(*(uint64_t *)&inp[2]);
                pos += 8;
                left_header_len_ = size >=LONG_HEADER ? 0 : LONG_HEADER - size;
                header_len_      = LONG_HEADER;
            }
            else {
                return -1; //解析错误
            }

            if (msg_masked) { //是否有mask
                mask_ = *((unsigned int*)(inp + pos));
            }
            else header_len_ -= 4; // 去掉 mask_ 的长度

            return left_header_len_ == 0 ?  header_len_ : -2;
        }

        /**
         * @brief 解析payload部分,解析的数据输出到outbuf
         */
        ws_frame_type parse_payload(const char* buf, size_t size, std::string& outbuf) {
            const unsigned char* inp = (const unsigned char*)(buf);
            if (payload_length_ > size) // payload_length_ 是在 parse_header 里得到的长度
                return ws_frame_type::WS_INCOMPLETE_FRAME; //末完成

            if (payload_length_ > outbuf.size()) { //调整outbuf 的长度
    ...
    private:
        ...
        size_t payload_length_      = 0; // payload的长度
        size_t left_payload_length_ = 0; 

        //size_t header_len_        = -1; // 没有成功
